...
> 版本：v4.0
> 日期：2026-05-25
> **v4.0 变更**：平台从 Coze 切换至 Dify。角色创建使用 Workflow，天道推演使用 Chatflow + Agent 节点（原生 function calling）。工具（灵根觉醒等）注册为 Dify Tool，Agent 自主判定调用时机。
...
| Tool | Code Tool | `awaken-spiritual-roots` | 灵根觉醒器 |
...
[代码: JSON 校验]
  ...
  逻辑: 清理代码块标记 → JSON 解析 → 字段完整性校验 → 添加 spiritual_roots: []
...
def main(llm_output: str):
    ...
    # 确保 spiritual_roots 为空数组（凡人模板）
    if "cultivation" in data:
        data["cultivation"]["spiritual_roots"] = []
...
详见 `05b-system-prompt-narrative.md`。核心要点：

- 单一 System Prompt，描述天道角色、叙事规则、输出格式
- 明确告知 Agent 拥有 `awaken-spiritual-roots` 工具：
  > 你拥有以下工具可用：
  > - `awaken-spiritual-roots`：灵根觉醒。当玩家经历测灵根、进入灵池、被探查根骨等场景时调用。仅在角色尚未觉醒灵根时调用。

### 5.3 工具注册（Dify Tool 配置）

在 Dify 中注册 `awaken-spiritual-roots` 为 **Code Tool**：

| 配置项 | 值 |
|--------|-----|
| 工具名称 | awaken-spiritual-roots |
| 类型 | Code Tool (Python) |
| 描述 | 觉醒角色的灵根。按概率表随机抽取灵根类型，返回灵根元素列表、类别、稀有度和叙事种子。仅在角色尚未觉醒灵根时调用。 |
| 输入参数 | `character_id` (number, required) — 角色 ID |
| 输出格式 | JSON |

工具实现代码见 `05c-tool-spiritual-roots.md`。
...
| 灵根觉醒并发 | 两次 LLM 调用 | Agent 内部循环，一次响应 |
| 会话管理 | 后端手动传 history | Dify conversation_id 自动维护 |
| 自部署 | ❌ | ✅ Docker |

---

*05-dify-workflows.md v4.0。关联：[05a](./05a-system-prompt-char-create.md) | [05b](./05b-system-prompt-narrative.md) | [05c](./05c-tool-spiritual-roots.md)。*
